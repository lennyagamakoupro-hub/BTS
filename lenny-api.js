/* ============================================
   ...
   ============================================ */
/* Jeton administrateur (formateur) — saisi UNE fois par Lenny, stocké
   localement sur son appareil. Jamais écrit dans la source ; envoyé en
   en-tête X-Admin-Token sur les routes admin du backend. */
window.LennyAdmin = (function () {
  const KEY = "lenny-admin-token";
  ...
  function ensure() {
    let t = token();
    if (!t) { t = (window.prompt("Jeton administrateur (formateur) :") || "").trim(); if (t) setToken(t); }
    return t;
  }
  ...
  return { token, setToken, ensure, headers, clear: () => setToken("") };
})();
...
window.LennyAPI = (function () {
  ...
  async function getFeedbackAdmin() {
    if (remote()) {
      try {
        if (window.LennyAdmin) window.LennyAdmin.ensure();
        const r = await fetch(base() + "/api/feedback/admin?user_id=" + encodeURIComponent(userId()), {
          headers: window.LennyAdmin ? window.LennyAdmin.headers() : {},
        });
        if (r.status === 401 || r.status === 503) {
          if (window.LennyAdmin) window.LennyAdmin.clear();
          try { alert("Accès formateur refusé : jeton administrateur invalide ou manquant."); } catch (e) {}
        } else if (r.ok) {
