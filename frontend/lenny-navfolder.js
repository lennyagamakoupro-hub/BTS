...
(function () {
  ...
  function init() {
    ...
    const ids = ["srs-btn", "quiz-general-btn", "exam-btn", "devoir-general-btn", "calc-btn"];
    ...
    function syncDot() {
      ...
      if (dot && badge) dot.hidden = badge.hidden;
    }
    syncDot();
    const badge = document.getElementById("srs-badge");
    if (badge) new MutationObserver(syncDot).observe(badge, { attributes: true, childList: true, subtree: true });
  }
