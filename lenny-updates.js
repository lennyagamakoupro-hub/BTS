...
(function () {
  ...
  async function addUpdate(entry) {
    ...
    if (remote()) {
      try {
        ...
        const r = await fetch(apiBase() + "/api/updates", {
          method: "POST",
          headers: window.LennyAdmin ? window.LennyAdmin.headers({ "Content-Type": "application/json" }) : { "Content-Type": "application/json" },
          body: JSON.stringify({
        ...
        if (r.status === 401 || r.status === 503) {
          ...
          try { alert("Publication refusée : jeton administrateur invalide."); } catch (e2) {}
        }
  ...
  async function removeUpdate(id) {
    if (remote()) {
      try {
        if (window.LennyAdmin) window.LennyAdmin.ensure();
        const r = await fetch(apiBase() + "/api/updates/" + encodeURIComponent(id), {
          method: "DELETE", headers: window.LennyAdmin ? window.LennyAdmin.headers() : {},
        });
        if (r.status === 401 || r.status === 503) {
          if (window.LennyAdmin) window.LennyAdmin.clear();
          try { alert("Suppression refusée : jeton administrateur invalide."); } catch (e) {}
        }
      } catch (e) {}
    }
